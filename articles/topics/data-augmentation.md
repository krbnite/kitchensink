# Data Augmentation

Associated article guide for data augmentation, robustness, synthetic examples,
sensor transformations, and train/test leakage concerns.

For broader repo context, see [Time Series](../../time-series/README.md),
[Deep Learning](../../deep-learning/README.md), and
[Health and Digital Biomarkers](../../health/README.md).

## Articles

- [The Many Faces of Data Leakage](../2019/2019-06-26-The-Many-Faces-of-Data-Leakage.md) (2019-06-26)
- [Nudged Box-Cox Time-Series Augmentation](../2020/2020-07-13-nudged-box-cox-time-series-augmentation.md) (2020-07-13)
- [Homotopic Time-Series Augmentations](../2020/2020-07-15-homotopic-time-series-augmentations.md) (2020-07-15)
- [Realistic Noise Injection For Time Series](../2020/2020-07-16-realistic-noise-injection-for-time-series.md) (2020-07-16)
- [Time-Series Data Augmentation Research Notes](../2020/2020-07-17-time-series-data-augmentation-research-notes.md) (2020-07-17)

## Later Cross-References

- Add the AlexNet internals article here after it is promoted from quarantine;
  its augmentation notes belong in this topic guide.
